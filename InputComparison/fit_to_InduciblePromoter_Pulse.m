function [out] = fit_to_InduciblePromoter_Pulse(epccOutputResultFileNameBase,epcc_exps,global_theta_guess)
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% fit_to_InduciblePromoter_Pulse script - - runs PE on the pulse class of inputs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    duration = 50*60;     % Duration of the experiment (minutes)
    numPulses = 50;       % Number of pulses    
    inducer = randi([0 1000],1,1);  % Extract 1 random integer from the Uniform distribution in (0,1000)uM to be used as the high value in each pulse

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Create a new experiment to simulate with the pulse input
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    clear newExps;
    newExps.n_exp = 1;                                         % Number of experimets
    newExps.n_obs{1}=1;                                        % Number of observables per experiment                         
    newExps.obs_names{1}=char('Fluorescence');                 % Name of observables per experiment    
    ...
    newExps.n_steps{1}=numPulses*2;                            % Number of steps in the input
    newExps.u{1}=repmat([inducer 0],1,numPulses);              % IPTG values for the input              
    newExps.t_con{1}=union(0:60:duration,10:60:duration);      % Switching times 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Parameter estimation (performed every 5 hours)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
